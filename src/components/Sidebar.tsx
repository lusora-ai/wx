    ...
    <aside id="sidebar-container" className="w-64 bg-white/80 backdrop-blur-md border-r border-apple-border flex flex-col h-full select-none">
      ...
      <div id="brand-header" className="p-6 pb-5 flex items-center space-x-3 border-b border-apple-border/50">
        ...
        <div className="flex flex-col">
          <span className="font-sans font-semibold text-sm tracking-tight text-apple-dark">小顺 AI 工作台</span>
          <span className="font-mono text-[9px] text-apple-muted tracking-wide">XIAOSHUN WORKBENCH</span>
        </div>
      </div>

      {/* Main Navigation Tab Container */}
      <nav id="sidebar-nav" className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
        <div className="text-[10px] uppercase tracking-wider text-apple-muted font-bold px-3 mb-2">主控制台</div>
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full group px-3 py-2 rounded-xl flex items-center justify-between text-left transition-all duration-200 cursor-pointer ${
                isActive 
                  ? 'bg-apple-bg text-apple-dark font-medium' 
                  : 'text-apple-muted hover:text-apple-dark hover:bg-apple-bg/55'
              }`}
            >
              <div className="flex items-center space-x-3">
                <IconComponent className={`h-[18px] w-[18px] transition-colors ${isActive ? 'text-apple-blue' : 'text-apple-muted group-hover:text-apple-dark'}`} />
                <div className="flex flex-col">
                  <span className={`text-xs font-medium ${isActive ? 'text-apple-dark' : 'text-apple-muted group-hover:text-apple-dark'}`}>{item.label}</span>
                  <span id={`sidebar-sub-${item.id}`} className="text-[9px] font-mono tracking-wide text-apple-muted/80">{item.sub}</span>
                </div>
              </div>
              {item.badge !== 0 && (
      ...
      <div id="sidebar-footer" className="p-4 border-t border-apple-border/50 bg-apple-bg/30 space-y-4">
        ...
        <div id="token-usage-bar" className="space-y-1.5 p-3 rounded-2xl bg-apple-bg">
          ...
          <div className="flex justify-between text-[9px] font-mono text-apple-muted">
        ...
        <div id="user-profile-card" className="flex items-center justify-between px-2">
          <div className="flex items-center space-x-2.5">
            ...
            <div className="flex flex-col">
              <span className="text-[11px] font-semibold text-apple-dark">顺子老师</span>
              <span className="text-[9px] text-apple-muted font-mono">CHIEF OPERATOR</span>
            </div>
          </div>
